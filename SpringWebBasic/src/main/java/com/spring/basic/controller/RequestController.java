...
public class RequestController {
	...
	/*
	 ...
	 req-ex01.jsp 파일을 열도록 메서드를 구성해 보세요
	 */
...
//	@RequestMapping(value="/request/basic01", method = RequestMethod.GET)
	...
	public String basicGet() {
		System.out.println("/basic01 GET 요청이 들어옴~!");
		return "request/req-ex01";
	}
...
//	@RequestMapping(value = "/request/basic01", method = RequestMethod.POST)
	...
	public String basicPost() {
		System.out.println("/basic01 POST 요청이 들어옴~!");
		return "request/req-ex01";
	}
	...
	// 요청 URI 주소가 같더라도, 전송 방식에 따라 맵핑이 다르게 하기 때문에
	// 같은 주소를 사용하는 것이 가능합니다. (GET -> 화면처리, POST -> 입력값 처리)
	
	/*
	 # 스프링에서 요청과 함께 전달된 데이터를 처리하는 방식 소개
...
//	@PostMapping("/join")
//	public void register( // @RequestParam("userId") 데이터    String userId 데이터를 받는 객체
...
//	}
	...
	/*
	 2. @RequestParam 아노테이션을 이요한 요청 파라미터처리.
	...
	public void register( // @RequestParam("userId") 데이터    String userId 데이터를 받는 객체
			...
			// @RequestParam 은 무조건 값이 와야 한다는 전제이다
			//required 는    hobby를 아무것도 체크하지 않았을 때에도 에러아님
			//defaultValue 기본값 설정 hobby를 설정하지 않았을 때  
	) {
		System.out.println("ID: " + userId);
		System.out.println("PW: " + userPw);
		System.out.println("hobby: " + hobbies);
	...
	*/
	...
	/*
	 ...
	 # 커맨드 객체: vo와 목적이 다름    사용자의 입력을 담기 위해 설계되고, 특정 검증 로직이나 비즈니스 로직을
