...
<body>
					...
					<div class="modal-con col-sm-4 col-xs-6">
						<div class="modal-inner">
							...
							<div class="content-inner">
								<p id="snsContent">` + board.content + `</p>
							</div>
	...
	<script>
		...
		function getLikeList(page, reset) {
			...
			localStorage 브라우저가 종료되더라도 데이터는 유지
							브라우저가 탭이 여러개 존재하더라도 데이터가 공유됩니다.
			sessionStorage 브라우저가 종료되면 데이터가 소멸됩니다.
							브라우저가 탭 별로 데이터가 저장되기 때문에 공유되지 않습니다.
			*/
		...
		$contentDiv.addEventListener('click', e => {
			...
			if(e.target.matches('.link-inner #delBtn')) {
				...
				//비동기 방식으로 삭제를 진행해 주세요. (삭제 버튼은 여러 개 입니다! 모달에도 있음)
				...
				// 	else{
				// 		alert('게시물이 정상적으로 삭제되었습니다.');
				// 		getList(1, true); // 삭제가 반영된 새로운 글 목록
				// 	}
				// })
			...
			if(e.target.matches('.link-inner #comment') || e.target.matches('.image-inner img')){
				$('#snsModal').modal('show');

				// document.querySelector('.snsImg').style.display = 'block';
        ...
        */
		...
		$contentDiv.addEventListener('click', e => {
			...
			.then(result => {
				...
				if(result === 'like'){
					...
					//firstchild = 줄개행 공백   firstElementChild = 공백없음  공백은 element가 아님
					e.target.style.color = 'blue';
					const $cnt = e.target.parentNode.previousElementSibling.children[1];
					// 이벤트 타겟의 부모의 형제의  자식전부 얻어서 두번째 자식 
			...
			})// 이벤트 타겟의 부모의 형제의 2번째 자식    자식전부 얻어서 두번째 자식 
		...
		// 무한 스크롤 페이지  브라우저 창에서 스크롤이 발생할 때마다 이벤트 발생. 
		window.addEventListener('scroll', () => {
			if(!reqStatus) handleScroll();
		});
		...
		$contentDiv.addEventListener('click', e => {
			e.preventDefault();
			if(!e.target.matches('#likstBtn')) return;
		})
