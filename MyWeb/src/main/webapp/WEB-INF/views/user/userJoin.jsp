<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <script>
        ...
        document.getElementById('idCheckBtn').onclick = function(){
            ...
            const url = `${pageContext.request.contextPath }/user/\${userId}`;
            console.log('url: ' + url);

            // 서버 요청 정보 설정
            xhr.open('GET', `${pageContext.request.contextPath }/user/\${userId}`); //const userId 이다
            xhr.send();
            ...
            */
           // 비동기는 여러개의 코드 순서에 상관없이 먼저 동기된 순서대로 실행하기 때문에 순서를 정해줘야 한다 
           // 순서가 섞여 에러가 된 것을 callback hell   fetch와 Promise가 그것을 막음

           /*
           //현재는 get 방식

           //fetch('url', {요청 관련 정보를 담은 객체(GET방식에서는 따로 전달 안함.)})
           // url에 데이터를 붙여서 보내든, json데이터를 보내든 여러 방법이 있다
           ...
           .then(res => {
                ...
                return res.text(); // 서버가 전달한 데이터를 다시 Promise형태로 리턴.
           })  // 패치함수를 이용해서 통신해봄
           ...
           .fetch(error => { // 통신 과정에서 에러가 발생했을 경우에만 실행되는 함수.
           ...
           */
           ...
           .then(res => res.text()) // 화살표 함수 내의 코드가 한 줄이고, 그것이 return이라면 괄호와 return 생략 가능
           // 요청 완료 후 응답 정보에서 텍스트 데이터가 담긴 Promise 반환.
           .then(data => { // 텍스트 데이터만 담긴 Promise 객체로부터 data를 전달받음.
                if(data === 'ok'){ //data에sms ok 와 duplicated 둘 중 하나가 들어있음
                    ...
                    document.getElementById('userId').focus(); // 입력창에 포커싱
                }
           }); 
        }// 아이디 중복 확인 끝
        ...
        document.querySelector('.mail-check-input').onblur = function(e) {
            ...
            new daum.Postcode({
                oncomplete: function(data) {
                    ...
                    document.getElementById('addrZipNum').value = data.zonecode; //zonecode: 우편번호
